...
namespace pimoroni {
  ...
  void __isr GalacticUnicorn::dma_complete() {
    ...
    //}
    if(unicorn != nullptr && dma_channel_get_irq1_status(audio_dma_channel)) {
      unicorn->next_audio_sequence();
    }
  }
  ...
  void GalacticUnicorn::dma_safe_abort(uint channel) {
    ...
    //hw_clear_bits(&dma_hw->inte0, irq1_save);
  ...
  // U/D/L/R and L(eave)/B(blank)/R(rotate)
  void GalacticUnicorn::scroll(char direction, size_t count, char replacement) {
    uint8_t dst_first = 0, dst_last_ex = 0;
    int8_t idx_delta = 0;
    int data_offset = 0;
  
    // row 0 column 0 is bottom right
    if (direction == 'D' || direction == 'U') {
      ...
      if (direction == 'D') {
        dst_first = 0u;
        dst_last_ex = HEIGHT - count;
        idx_delta = int8_t(count);
      } else {
        dst_first = HEIGHT - count;
        dst_last_ex = 0u;
        idx_delta = int8_t(0 - count);
      }
      data_offset = ROW_BYTES * idx_delta;
      for (auto idx = dst_first; idx != dst_last_ex; idx += idx_delta) {
        uint8_t *dst = &bitstream[idx * ROW_BYTES + 2];
        uint8_t *src = dst + data_offset;
        for(uint8_t frame = 0; frame < BCD_FRAME_COUNT; frame++) {
          std::memcpy(dst,
                      src,
                      WIDTH);
